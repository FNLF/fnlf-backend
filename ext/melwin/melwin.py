...
class Melwin():
		...
		for data in response:
			...
			for v in data[1][0]:
				...
				if key == 'aClubMembers':
					...
					n = "%s-12-31" % v
					d[i]['membership'].update({'valid': datetime.datetime.combine(datetime.datetime.strptime(n, '%Y-%m-%d'), datetime.datetime.max.time())})
				elif key == 'aSaldo':
	...
	def get_member_licenses(self, member):
		...
		try:
			if len(response[1][0]) > 0:
				...
				if isinstance(response[5][0][0], datetime.date):
					licenses.update({'expiry': datetime.datetime.combine(response[5][0][0], datetime.datetime.max.time())})
			else:
				pass
		...
		if 'expiry' not in licenses:
			licenses.update({'expiry': datetime.datetime.combine(datetime.date((datetime.datetime.now().year - 1), 12, 31),
				...
				if self.do_geo:
					try:
						...
						if geo != None:
							...
							curr_members[int(member)]['location'].update({'geo_importance': float(geo.raw['importance'])})
